...
type GoogleTokenBundle = { accessToken: string; refreshToken?: string; expiresAt?: number; scopes?: string[]; accountId?: string; accountEmail?: string };
...
export class AgentRuntime {
  ...
  async run(agentId: string, identity: Identity, input: AgentMessageInput): Promise<AgentMessageOutput> {
    ...
    try { authorization = await this.credits.reserve({ requestId, workspaceId: identity.workspaceId, agentId: agent.id, maximumCredits: 1 }); } catch (error) {
      if (error instanceof CreditError && error.code === "INSUFFICIENT_CREDITS") await this.store.addUsageEvent({ requestId, workspaceId: identity.workspaceId, agentId: agent.id, conversationId: currentConversation.id, model: agent.model, inputTokens: 0, outputTokens: 0, toolCalls: 0, credits: 0, status: "insufficient_credits", channel, createdAt: new Date().toISOString() });
      throw error;
    }
    try {
      ...
      const storedConnections = (await Promise.all(connections.map((connection) => this.store.getConnection(connection.id, agent.id, identity.workspaceId)))).filter((connection): connection is StoredAgentConnection => Boolean(connection));
      const customConnections = storedConnections.filter((connection) => connection.kind === "custom_api");
      const gmailConnections = storedConnections.filter((connection) => connection.kind === "native" && connection.provider === "google_gmail" && connection.status === "connected");
      const customToolMap = new Map(customConnections.map((connection) => [customToolName(connection), connection]));
      const tools: ToolDefinition[] = agent.level >= 3 ? [
        ...agent.enabledTools.map((name) => BUILTIN_TOOLS[name]).filter(Boolean),
        ...(agent.enabledTools.includes("send_email") && gmailConnections.some((connection) => connection.permissions.includes("https://www.googleapis.com/auth/gmail.send")) ? [BUILTIN_TOOLS.send_email] : []),
        ...customConnections.map(customConnectionTool),
      ] : [];
      ...
      if (explicitContact) {
        ...
        toolCalls = 1; messages.push({ role: "assistant", content: "" }); messages.push({ role: "tool", content: result.output, tool_call_id: `direct_capture_${requestId}`, name: "capture_contact" });
        ...
        toolCalls = 1; messages.push({ role: "assistant", content: "" }); messages.push({ role: "tool", content: result.output, tool_call_id: `direct_custom_api_${requestId}`, name: customToolName(explicitCustomConnection) });
      ...
      for (let round = 0; round < config.maxToolRounds && completion.toolCalls.length; round += 1) {
        ...
        for (const call of completion.toolCalls) {
          ...
          let result: { output: string; handoff: boolean };
          if (customConnection) result = await executeCustomApiConnection(customConnection, call.function.arguments);
          else if (call.function.name === "send_email") {
            const gmail = gmailConnections[0];
            if (!gmail || !agent.enabledTools.includes("send_email")) result = { output: JSON.stringify({ ok: false, error: "Email sending is not approved or no connected Gmail account is available." }), handoff: false };
            else result = await executeGmailSend(gmail, call.function.arguments);
          } else result = await executeTool(call.function.name, call.function.arguments);
...
async function executeGmailSend(connection: StoredAgentConnection, rawArguments: string): Promise<{ output: string; handoff: boolean }> {
  let args: Record<string, unknown>;
  try { args = JSON.parse(rawArguments) as Record<string, unknown>; } catch { return { output: JSON.stringify({ ok: false, error: "Invalid email arguments." }), handoff: false }; }
  const to = typeof args.to === "string" ? args.to.trim() : "";
  const subject = typeof args.subject === "string" ? args.subject.trim() : "";
  const body = typeof args.body === "string" ? args.body.trim() : "";
  if (!isEmail(to) || !subject || !body) return { output: JSON.stringify({ ok: false, error: "A valid recipient, subject, and email body are required." }), handoff: false };
  if (subject.length > 998 || body.length > 100000) return { output: JSON.stringify({ ok: false, error: "Email subject or body is too long." }), handoff: false };
  if (/[\r\n]/.test(subject) || /[\r\n]/.test(to)) return { output: JSON.stringify({ ok: false, error: "Email headers contain invalid characters." }), handoff: false };
  if (!connection.encryptedSecret) return { output: JSON.stringify({ ok: false, error: "The Gmail connection has no stored authorization." }), handoff: false };
  let token = JSON.parse(openSecret(connection.encryptedSecret)) as GoogleTokenBundle;
  if (!token.accessToken) return { output: JSON.stringify({ ok: false, error: "The Gmail authorization is invalid. Please reconnect Gmail." }), handoff: false };
  if (token.expiresAt && token.expiresAt <= Date.now() + 60000) {
    if (!token.refreshToken || !config.googleClientId || !config.googleClientSecret) return { output: JSON.stringify({ ok: false, error: "The Gmail authorization expired. Please reconnect Gmail." }), handoff: false };
    const refreshed = await refreshGoogleAccessToken(token.refreshToken);
    token = { ...token, accessToken: refreshed.accessToken, expiresAt: refreshed.expiresAt, refreshToken: refreshed.refreshToken ?? token.refreshToken };
  }
  const rawMessage = [`To: ${to}`, `Subject: ${subject}`, "Content-Type: text/plain; charset=UTF-8", "MIME-Version: 1.0", "", body.replace(/\r?\n/g, "\r\n")].join("\r\n");
  const response = await fetch("https://gmail.googleapis.com/gmail/v1/users/me/messages/send", { method: "POST", headers: { authorization: `Bearer ${token.accessToken}`, "content-type": "application/json", accept: "application/json" }, body: JSON.stringify({ raw: Buffer.from(rawMessage, "utf8").toString("base64url") }), signal: AbortSignal.timeout(15000) });
  const data = await response.json().catch(() => ({})) as Record<string, unknown>;
  if (!response.ok) return { output: JSON.stringify({ ok: false, status: response.status, error: "Gmail rejected the message.", detail: typeof data.error_description === "string" ? data.error_description : typeof data.error === "object" ? "Google returned an error." : undefined }), handoff: false };
  return { output: JSON.stringify({ ok: true, action: "email_sent", messageId: typeof data.id === "string" ? data.id : undefined, to, subject }), handoff: false };
}
async function refreshGoogleAccessToken(refreshToken: string): Promise<{ accessToken: string; expiresAt?: number; refreshToken?: string }> {
  const response = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded", accept: "application/json" }, body: new URLSearchParams({ grant_type: "refresh_token", refresh_token: refreshToken, client_id: config.googleClientId!, client_secret: config.googleClientSecret! }).toString(), signal: AbortSignal.timeout(10000) });
  const data = await response.json().catch(() => ({})) as Record<string, unknown>;
  if (!response.ok || typeof data.access_token !== "string") throw new Error("Gmail authorization could not be refreshed. Please reconnect Gmail.");
  return { accessToken: data.access_token, expiresAt: typeof data.expires_in === "number" ? Date.now() + data.expires_in * 1000 : undefined, refreshToken: typeof data.refresh_token === "string" ? data.refresh_token : undefined };
}
function isEmail(value: string): boolean { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) && value.length <= 320; }
function extractExplicitContactRequest(message: string): { name: string; email: string; phone: string; note: string } | undefined { const email = message.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i)?.[0]; const asksForFollowUp = /\b(callback|call me|follow[ -]?up|contact me|reach me|sales team|speak with sales)\b/i.test(message); if (!email || !asksForFollowUp) return undefined; const name = message.match(/\b(?:my name is|i am|i'm)\s+([A-Z][A-Za-z'-]+(?:\s+[A-Z][A-Za-z'-]+){0,3})/i)?.[1]?.trim() || "Visitor"; const phone = message.match(/(?:\+?\d[\d\s().-]{7,}\d)/)?.[0]?.trim() || ""; return { name, email, phone, note: message.trim().slice(0, 240) }; }
