...
class PostgresStore implements Store {
  ...
  async adminAgents(limit: number) { const result = await this.pool.query("SELECT a.*, COUNT(DISTINCT k.id)::int AS knowledge_count, COUNT(DISTINCT c.id)::int AS conversation_count, COUNT(DISTINCT u.request_id) FILTER (WHERE u.status='completed')::int AS responses, COALESCE(SUM(u.credits),0)::int AS credits_used, COUNT(DISTINCT d.id)::int AS deployment_count FROM gbolix_agents a LEFT JOIN gbolix_knowledge k ON k.agent_id=a.id LEFT JOIN gbolix_conversations c ON c.agent_id=a.id LEFT JOIN gbolix_usage_events u ON u.agent_id=a.id LEFT JOIN gbolix_deployments d ON d.agent_id=a.id GROUP BY a.id ORDER BY a.updated_at DESC LIMIT $1", [limit]); return result.rows.map((row) => ({ ...agentRow(row), knowledgeCount: row.knowledge_count, conversationCount: row.conversation_count, responses: row.responses, creditsUsed: row.credits_used, deploymentCount: row.deployment_count })); }
